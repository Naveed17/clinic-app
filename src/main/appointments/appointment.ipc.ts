...
export function registerAppointmentIpc(io?: SocketIOServer): void {
  ...
  ipcMain.handle('appointments:create', async (_, input) => {
    ...
    if (io) emitNotification(io, { kind: 'success', title: 'Appointment created', message: 'A new appointment was scheduled.', payload: { entity: 'appointment', id: appointment.id } });
    return appointment;
  });
  ipcMain.handle('appointments:update', async (_, id, input) => {
    const appointment = await updateAppointment(id, input);
    if (io) emitNotification(io, { kind: 'info', title: 'Appointment updated', message: 'An appointment was updated.', payload: { entity: 'appointment', id: appointment.id } });
    return appointment;
  });
  ipcMain.handle('appointments:cancel', async (_, id) => {
    const appointment = await cancelAppointment(id);
    if (io) emitNotification(io, { kind: 'warning', title: 'Appointment cancelled', message: 'An appointment was cancelled.', payload: { entity: 'appointment', id: appointment.id } });
    return appointment;
  });
  ipcMain.handle('appointments:updateStatus', async (_, id, status) => {
    const appointment = await updateAppointmentStatus(id, status);
    if (io) emitNotification(io, { kind: 'info', title: 'Appointment updated', message: `Appointment status changed to ${appointment.status}.`, payload: { entity: 'appointment', id: appointment.id } });
    return appointment;
  });
}
